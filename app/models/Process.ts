...
// Compound indexes
ProcessSchema.index({ 'adoptedByDomains.domainId': 1 });
ProcessSchema.index({ category: 1, executionModel: 1 });
ProcessSchema.index({ 'adoptedByDomains.domainId': 1, 'adoptedByDomains.isActive': 1 });
ProcessSchema.index({ name: 'text', description: 'text' });

const Process = mongoose.models.Process || mongoose.model<IProcess>('Process', ProcessSchema);
